...
const createTypeIdSchema = (prefix: string) =>
  z.string().refine(
    (val) => {
      try {
...
server.tool(
  ...
  async () => {
    ...
    const formatted = teams
      .map(
        (team) =>
          `• ${team.name} (ID: \`${team.id})\`${team.isDefault ? " [default]" : ""}`,
      )
      .join("\n");
...
server.tool(
  ...
  {
    teamId: createTypeIdSchema("team").describe(
      "The ID of the team to list sites for",
    ),
  },
  async ({ teamId }) => {
  ...
  async ({ siteId }) => {
    ...
    if (!site) {
      return {
        ...
        content: [
          {
            type: "text",
            text: "Failed to retrieve site from Arcjet API.",
          },
        ],
      };
    }
    ...
    return {
      content: [
        {
          type: "text",
          text: `Key for ${site.name}: \`${site.key}\``,
        },
      ],
    };
  },
